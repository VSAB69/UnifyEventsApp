...
final tempBookingsProvider = FutureProvider.autoDispose
    .family<List<dynamic>, int>((ref, cartItemId) async {
      final dio = ref.read(dioProvider);
      try {
        final response = await dio.get(
          '/tempbookings/',
          queryParameters: {'cart_item': cartItemId},
        );
        List<dynamic> all = [];
...
final tempTimeslotsProvider = FutureProvider.autoDispose
    .family<List<dynamic>, int>((ref, cartItemId) async {
      final dio = ref.read(dioProvider);
      try {
        final response = await dio.get(
...
class CartActionService {
  ...
  Future<void> updateTimeSlot(Map<String, dynamic> data) async {
    await _dio.post(
      '/temp-timeslots/',
      data: data,
    ); // POST creates or updates temp slot logic (handled by backend or we must specify cart_item)
  }
}
